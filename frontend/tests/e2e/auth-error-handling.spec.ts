...
test.describe('Authentication Error Handling', () => {
  ...
  test('handles missing Clerk publishable key', async ({ page }) => {
    ...
    const hasExpectedError = pageErrors.some(error =>
  ...
  test('handles Clerk initialization failures', async ({ page }) => {
    ...
    const authError = consoleLogs.some(log =>
      log.text.includes('Clerk') && log.type === 'error'
    )
  ...
  test('displays user-friendly error messages', async ({ page }) => {
    ...
    const loadingElements = await page.locator('text=Loading, text=authenticat').count()
  ...
  test('auth state recovery after errors', async ({ page }) => {
    ...
    const authRecoveryLogs = consoleLogs.filter(log =>
  ...
  test('token refresh error handling', async ({ page }) => {
    ...
    const tokenErrors = consoleLogs.filter(log =>
