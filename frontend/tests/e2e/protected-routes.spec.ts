...
test.describe("Protected Routes Security", () => {
  ...
  test("direct navigation to protected routes fails", async ({ page }) => {
    ...
    const testCases = [
      ...
      },
      {
        ...
        description: "Settings direct access",
      },
    ]
  ...
  test("auth wrapper renders protection UI", async ({ page }) => {
    ...
    const wrapperLogs = consoleLogs.filter(
      (log) =>
        log.includes("[ClerkAuthWrapper]") || log.includes("[ProtectedRoute]"),
    )

    console.log("[Test] Auth wrapper logs:", wrapperLogs.slice(0, 3))
  })

  test("authentication state changes trigger route updates", async ({
    page,
  }) => {
    ...
    const authStateLogs = consoleLogs.filter(
      (log) =>
...
test.describe("Route Context and Data", () => {
  ...
  test("navigation between protected routes works when authenticated", async ({
    page,
  }) => {
