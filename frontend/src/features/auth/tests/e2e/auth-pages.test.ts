...
test.describe('Auth Pages E2E', () => {
  ...
  test('successful authentication allows access to protected routes', async ({ page }) => {
    ...
    test.skip(!process.env.PLAYWRIGHT_TEST_EMAIL, 'No test credentials provided')
    ...
    await expect(page.locator('[data-clerk-element="userProfile"]')).toBeVisible()
  })
  ...
  test('admin user list page loads for authenticated users', async ({ page }) => {
    test.skip(!process.env.PLAYWRIGHT_TEST_EMAIL, 'No test credentials provided')
