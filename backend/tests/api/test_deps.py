...
class TestGetCurrentUserSession:
    ...
    def test_successful_authentication(
        ...
        mock_clerk_service.return_value.authenticate_session.assert_called_once_with(
            mock_request
        )
        mock_session.exec.assert_called_once()

    @patch("app.api.deps.ClerkService")
    def test_authentication_no_user_id(
        self, mock_clerk_service, mock_request, mock_session
    ):
    ...
    @patch("app.api.deps.ClerkService")
    @patch("app.api.deps.UserSyncService")
    def test_user_not_found_with_sync(
        self,
        mock_sync_service,
        mock_clerk_service,
        mock_request,
        ...
        # Assertions
        assert result == mock_user
        mock_sync_service.return_value.fetch_and_sync_user.assert_called_once_with(
            "clerk_123"
        )

    @patch("app.api.deps.ClerkService")
    def test_inactive_user(
        self, mock_clerk_service, mock_request, mock_session, mock_user
    ):
    ...
    @patch("app.api.deps.ClerkService")
    def test_clerk_authentication_error(
        self, mock_clerk_service, mock_request, mock_session
    ):
        """Test handling of Clerk authentication errors"""
        # Setup mock to raise ClerkAuthenticationError
        mock_clerk_service.return_value.authenticate_session.side_effect = (
...
class TestRequireRole:
    ...
    def test_single_role_success(
        self,
        mock_role_service,
        mock_user_role_service,
        mock_user,
        mock_session,
        mock_user_role,
        mock_role,
        ...
        # Assertions
        assert result == mock_user
        mock_user_role_service.return_value.get_user_roles.assert_called_once_with(
            mock_session, mock_user.id
        )
        mock_role_service.return_value.get_role.assert_called_once_with(
            mock_session, mock_user_role.role_id
        )

    @patch("app.api.deps.UserRoleService")
    @patch("app.api.deps.RoleService")
    def test_multiple_roles_success(
        self,
        mock_role_service,
        mock_user_role_service,
        mock_user,
...
class TestDependencyIntegration:
    ...
    @patch("app.api.deps.UserRoleService")
    @patch("app.api.deps.RoleService")
    def test_admin_user_with_app_owner_role(
        self, mock_role_service, mock_user_role_service
    ):
    ...
    @patch("app.api.deps.UserRoleService")
    @patch("app.api.deps.RoleService")
    def test_admin_user_with_platform_admin_role(
        self, mock_role_service, mock_user_role_service
    ):
