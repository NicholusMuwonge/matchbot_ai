...
def test_clerk_service_logging():
    ...
    try:
        from app.services.clerk_auth import ClerkService, ClerkAuthenticationError
        ...
        try:
            ...
            try:
                result = service.validate_session_token("fake_session_token_for_testing")
...
def show_logging_configuration():
    ...
    print(f"   CLERK_SECRET_KEY: {'✅ Set' if os.getenv('CLERK_SECRET_KEY') else '❌ Not set'}")
    
    # Show active loggers
    clerk_service_logger = logging.getLogger("clerk_service")
    clerk_api_logger = logging.getLogger("clerk_backend_api")
    
    print(f"   clerk_service logger level: {logging.getLevelName(clerk_service_logger.level)}")
    print(f"   clerk_backend_api logger level: {logging.getLevelName(clerk_api_logger.level)}")
