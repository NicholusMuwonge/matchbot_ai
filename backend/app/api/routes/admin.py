...
from app.api.deps import ClerkSessionSuperuser
...
async def create_sign_in_token(
    token_data: SignInTokenCreate, _: ClerkSessionSuperuser
) -> SignInTokenResponse:
    try:
...
async def create_jwt_template(
    template_data: JWTTemplateCreate, _: ClerkSessionSuperuser
) -> JWTTemplateResponse:
    try:
        clerk_service = ClerkService()
...
async def verify_oauth_token(
    token_data: OAuthTokenVerify, _: ClerkSessionSuperuser
) -> OAuthTokenResponse:
    try:
        clerk_service = ClerkService()
...
@router.get("/health")
async def admin_health_check(_: ClerkSessionSuperuser) -> dict[str, str]:
    try:
        clerk_service = ClerkService()
