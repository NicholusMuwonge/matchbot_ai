...
from app.api.deps import AdminUser, AppOwnerUser, ClerkSessionUser, SessionDep
    ...
    try:
        ...
        if user.clerk_user_id:
            try:
                ...
                if remaining_roles:
                    remaining_role = role_service.get_role(session, remaining_roles[0].role_id)
                    new_role_name = remaining_role.name if remaining_role else "regular_user"
                else:
