...
def get_mock_user_session(request: Request, session: Session) -> User:
    ...
    if not settings.ENABLE_AUTH_TESTING:
        raise HTTPException(
            status_code=500, 
            detail="Test authentication is not enabled"
        )
    ...
    for role_name, user_data in TestUsers.USERS.items():
        ...
        if role:
            ...
            if not any(ur.role_id == role.id for ur in existing_role):
                ...
                created_users.append(f"Assigned {role_name} role to {user_data['email']}")
            else:
                created_users.append(f"{user_data['email']} already has {role_name} role")
